	...
	private class MissionSide
	{
		...
		public void SpawnTroops(Dictionary<int, Dictionary<int, AreaData>> areaMarkerDictionary, int spawnCount)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				List<KeyValuePair<int, AreaData>> list2 = areaMarkerDictionary.ElementAt(i % areaMarkerDictionary.Count).Value.ToList();
				...
				{
					int num = 1000 * item.Value.GetAvailableMachines(flag).Count() + item.Value.GetAvailableMachines(!flag).Count();
					list3.Add((new KeyValuePair<int, AreaData>(item.Key, item.Value), num));
				}
