...
public class TacticDefendCastle : TacticComponent
{
	...
	private bool IsSallyOutApplicable()
	{
		...
		{
			if (team.Side.GetOppositeSide() != 0)
			{
				continue;
			}
	...
	private void BalanceLaneDefenders(List<Formation> defenderFormations, out bool transferOccurred)
	{
		...
		if (source.Any() && (float)num7 * source.Min() <= (float)num4)
		{
			num7 = num6;
			num4 = TaleWorlds.Library.MathF.Max((int)((float)num7 * 0.1f), 1);
		}
		...
		int num9 = TaleWorlds.Library.MathF.Max((int)((float)defenderFormations.Sum((Formation df) => df.CountOfUnits) * 0.2f), 1);
		foreach (Formation receiverDefenderFormation in defenderFormations)
		{
			...
			foreach (Formation item in defenderFormations.Where((Formation df) => df != receiverDefenderFormation))
			{
				...
				{
					int num10 = TaleWorlds.Library.MathF.Min(-array5[side2], array5[side3]);
					array5[side2] += num10;
					array5[side3] -= num10;
					item.TransferUnits(receiverDefenderFormation, num10);
	...
	private void CarryOutDefense(List<SiegeLane> defendedLanes, List<SiegeLane> lanesToBeRetaken, bool isEnemyInside, bool doRangedJoinMelee, out bool hasTransferOccurred)
	{
		...
		foreach (ArcherPosition remainingArcherPosition2 in list19)
		{
			...
			Formation formation12 = list20.FirstOrDefault((Formation rrf) => remainingArcherPosition2.IsArcherPositionRelatedToSide(rrf.AI.Side));
			if (formation12 == null)
			{
			...
			if (formation12 != null)
			{
				formation12.AI.Side = remainingArcherPosition2.GetArcherPositionClosestSide();
				formation12.AI.ResetBehaviorWeights();
				TacticComponent.SetDefaultBehaviorWeights(formation12);
				formation12.AI.SetBehaviorWeight<BehaviorShootFromCastleWalls>(1f).ArcherPosition = remainingArcherPosition2.Entity;
				remainingArcherPosition2.SetLastAssignedFormation(base.Team.TeamIndex, formation12);
		...
		{
			Formation formation13 = list20.FirstOrDefault((Formation rrf) => remainingArcherPosition.IsArcherPositionRelatedToSide(rrf.AI.Side));
			if (formation13 == null)
			{
			...
			if (formation13 != null)
			{
				formation13.AI.Side = remainingArcherPosition.GetArcherPositionClosestSide();
				formation13.AI.ResetBehaviorWeights();
				TacticComponent.SetDefaultBehaviorWeights(formation13);
				formation13.AI.SetBehaviorWeight<BehaviorShootFromCastleWalls>(1f).ArcherPosition = remainingArcherPosition.Entity;
				remainingArcherPosition.SetLastAssignedFormation(base.Team.TeamIndex, formation13);
