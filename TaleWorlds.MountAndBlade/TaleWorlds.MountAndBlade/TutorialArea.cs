...
public class TutorialArea : MissionObject
{
	...
	{
		foreach (TutorialEntity item2 in _tagWeapon)
		{
			if (resetDestructibles)
			{
				foreach (DestructableComponent destructableComponent in item2.DestructableComponents)
				{
			...
			foreach (Tuple<GameEntity, MatrixFrame> entity in item2.EntityList)
			{
				if (!entity.Item1.HasScriptOfType<DestructableComponent>())
				{
					...
					GameEntity item = entity.Item1;
					MatrixFrame frame = entity.Item2;
