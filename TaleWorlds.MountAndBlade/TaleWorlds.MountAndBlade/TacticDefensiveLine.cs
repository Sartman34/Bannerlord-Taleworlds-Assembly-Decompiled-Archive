...
public class TacticDefensiveLine : TacticComponent
{
	...
	{
		if (tacticalPosition.TacticalRegionMembership != 0)
		{
			return 1f;
		}
	...
	private void DetermineMainDefensiveLine()
	{
		...
		IEnumerable<(TacticalPosition, float)> second = from tpftr in base.Team.TeamAI.TacticalRegions.SelectMany((TacticalRegion r) => ExtractPossibleTacticalPositionsFromTacticalRegion(r))
			where (tpftr.TacticalPositionType == TacticalPosition.TacticalPositionTypeEnum.Regional || tpftr.TacticalPositionType == TacticalPosition.TacticalPositionTypeEnum.HighGround) && IsTacticalPositionEligible(tpftr)
