...
public sealed class MissionNetworkComponent : MissionNetwork
{
	...
	private void HandleServerEventCreateFreeMountAgentEvent(GameNetworkMessage baseMessage)
	{
		...
		Vec3 initialPosition = createFreeMountAgent.Position;
		Vec2 initialDirection = createFreeMountAgent.Direction.Normalized();
	...
	private void HandleServerEventCreateAgent(GameNetworkMessage baseMessage)
	{
		...
		Vec2 direction = createAgent.Direction.Normalized();
		AgentBuildData agentBuildData3 = agentBuildData2.InitialDirection(in direction).MissionEquipment(createAgent.MissionEquipment).Team(teamFromTeamIndex)
	...
	private void SendAgentVisualsToPeer(NetworkCommunicator networkPeer, Team peerTeam)
	{
			...
			select pr)
		{
			if (item.Team == peerTeam)
			{
				...
				{
					PeerVisualsHolder visuals = item.GetVisuals(i);
					...
					AgentBuildData agentBuildData2 = agentBuildData.InitialDirection(in direction).IsFemale(agentVisuals.GetIsFemale()).BodyProperties(agentVisuals.GetBodyProperties());
					networkPeer.GetComponent<MissionPeer>();
					GameNetwork.BeginModuleEventAsServer(networkPeer);
					GameNetwork.WriteMessage(new CreateAgentVisuals(item.GetNetworkPeer(), agentBuildData2, item.SelectedTroopIndex));
					GameNetwork.EndModuleEventAsServer();
				}
			}
		}
