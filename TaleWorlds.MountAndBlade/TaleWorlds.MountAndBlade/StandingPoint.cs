	...
	private float GetPathDistance(Agent agent, ref WorldPosition userPosition, ref WorldPosition agentPosition)
	{
		...
		if (_cachedAgentDistances.TryGetValue(agent, out var value))
		{
			...
			else
			{
				...
				AgentDistanceCache agentDistanceCache = default(AgentDistanceCache);
				agentDistanceCache.AgentPosition = agentPosition.AsVec2;
				agentDistanceCache.StandingPointPosition = userPosition.AsVec2;
				agentDistanceCache.PathDistance = pathDistance;
				value = agentDistanceCache;
