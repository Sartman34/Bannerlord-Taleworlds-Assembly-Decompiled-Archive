...
public struct ArrangementOrder
{
	...
	{
		float num = TaleWorlds.Library.MathF.Max(1f, TaleWorlds.Library.MathF.Max(formation.Depth, formation.Width) * 0.8f);
	...
	private static void TransposeLineFormation(Formation formation)
	{
		...
		formation.SetPositioning(formation.GetReadonlyMovementOrderReference().CreateNewOrderWorldPosition(formation, WorldPosition.WorldPositionEnforcedCache.None));
	...
	private bool IsStrategicAreaClose(StrategicArea strategicArea, Formation formation)
	{
		if (strategicArea.IsUsableBy(formation.Team.Side))
		{
			...
			return worldPosition.DistanceSquaredWithLimit(in targetPoint, strategicArea.DistanceToCheck * strategicArea.DistanceToCheck + 1E-05f) < strategicArea.DistanceToCheck * strategicArea.DistanceToCheck;
		}
		return false;
	}
	...
	public static int GetArrangementOrderDefensivenessChange(ArrangementOrderEnum previousOrderEnum, ArrangementOrderEnum nextOrderEnum)
	{
		...
		{
			if (nextOrderEnum != 0 && nextOrderEnum != ArrangementOrderEnum.ShieldWall && nextOrderEnum != ArrangementOrderEnum.Square)
			{
				return -1;
			}
			return 0;
