...
public class TacticSallyOutHitAndRun : TacticComponent
{
	...
	protected override void ManageFormationCounts()
	{
		...
		for (int i = 0; i < _cavalryFormations.Count - (flag3 ? 1 : 0); i++)
		{
			if (list.Count > 0)
			{
				_cavalryFormations[i].AI.Side = list[i % list.Count].WeaponSide;
			}
			else
			{
				_cavalryFormations[i].AI.Side = FormationAI.BehaviorSide.Middle;
			}
		}
		for (int j = 0; j < _archerFormations.Count - (flag3 ? 1 : 0); j++)
		{
			if (list.Count > 0)
			{
				_archerFormations[j].AI.Side = list[j % list.Count].WeaponSide;
			}
			else
			{
				_archerFormations[j].AI.Side = FormationAI.BehaviorSide.Middle;
			}
		}
	...
	private void CheckAndChangeState()
	{
		...
		case TacticState.DestroyingSiegeWeapons:
		{
			...
			if (flag || flag2)
			{
				...
				TacticalDecision decision = new TacticalDecision(this, 31);
				teamAI.NotifyTacticalDecision(in decision);
			}
			break;
		}
