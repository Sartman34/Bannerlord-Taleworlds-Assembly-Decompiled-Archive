...
public abstract class TeamAISiegeComponent : TeamAIComponent
{
		...
		: base(currentMission, currentTeam, thinkTimerTime, applyTimerTime)
	{
		...
		for (i = 0; i < 3; i++)
		{
			...
			SiegeLanes[i].SetPrimarySiegeWeapons((from psw in PrimarySiegeWeapons
				where psw.WeaponSide == (FormationAI.BehaviorSide)i
				select psw into um
				select (um)).ToList());
			SiegeLanes[i].SetDefensePoints((from ckp in CastleKeyPositions
		...
		if (!_noProperLaneRemains)
		{
			...
			if (SiegeLanes.Count == 0)
			{
					...
					select ((CastleGate)ckp).DefenseSide)
				{
					...
					siegeLane2.SetDefensePoints((from ckp in CastleKeyPositions
