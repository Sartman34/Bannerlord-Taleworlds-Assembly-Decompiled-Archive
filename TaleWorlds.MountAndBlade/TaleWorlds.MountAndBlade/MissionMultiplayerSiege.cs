...
public class MissionMultiplayerSiege : MissionMultiplayerGameModeBase, IAnalyticsFlagInfo, IMissionBehavior
{
	...
	{
		int num = 0;
		bool flag2 = _masterFlagBestAgent != null && _masterFlagBestAgent.Team.Side == side;
		if (side == BattleSideEnum.Attacker)
		{
			...
			foreach (FlagCapturePoint item in AllCapturePoints.Where((FlagCapturePoint flag) => flag != _masterFlag && !flag.IsDeactivated && flag.IsFullyRaised && GetFlagOwnerTeam(flag).Side == BattleSideEnum.Attacker))
			{
		...
		else if (_masterFlag.IsFullyRaised)
		{
			if (GetFlagOwnerTeam(_masterFlag).Side == BattleSideEnum.Attacker)
			{
				...
				{
					int num2 = 0;
					for (int i = 0; i < AllCapturePoints.Count; i++)
					{
