...
public static class GameNetwork
{
	...
	internal static void FindGameNetworkMessages()
	{
		...
		for (int j = 0; j < list.Count; j++)
		{
			...
			_gameNetworkMessageTypesFromClient.Add(type, j);
			_gameNetworkMessageTypesAll.Add(type, j);
			_fromClientMessageHandlers.Add(j, new List<object>());
			_fromClientBaseMessageHandlers.Add(j, new List<GameNetworkMessage.ClientMessageHandlerDelegate<GameNetworkMessage>>());
		}
		...
		for (int k = 0; k < list2.Count; k++)
		{
			...
			_gameNetworkMessageTypesFromServer.Add(type2, k);
			_gameNetworkMessageTypesAll.Add(type2, k);
			_fromServerMessageHandlers.Add(k, new List<object>());
			_fromServerBaseMessageHandlers.Add(k, new List<GameNetworkMessage.ServerMessageHandlerDelegate<GameNetworkMessage>>());
		}
