...
public class PrefabDatabindingExtension : PrefabExtension
{
	...
	protected override void AfterAttributesSet(WidgetCreationData widgetCreationData, WidgetInstantiationResult widgetInstantiationResult, Dictionary<string, WidgetAttributeTemplate> parameters)
	{
		...
		foreach (WidgetAttributeTemplate item in attributesOf)
		{
			...
			{
				WidgetAttributeTemplate widgetAttributeTemplate = dictionary2[key2];
				string text = null;
				...
				{
					gauntletView.BindCommand(key2, new BindingPath(value4), text);
				}
				else
				{
					gauntletView.BindCommand(key2, new BindingPath(value4));
				}
			}
			else
			{
				...
				gauntletView.BindCommand(key2, new BindingPath(path2));
			}
		}
		foreach (WidgetAttributeTemplate allAttribute in template.AllAttributes)
		{
			...
			{
				gauntletView.BindData(key3, new BindingPath(value5));
			}
			else
			{
				...
				string key4 = value5;
				if (!parameters.ContainsKey(key4))
				{
					continue;
				}
				WidgetAttributeTemplate widgetAttributeTemplate2 = parameters[key4];
				if (widgetAttributeTemplate2.ValueType is WidgetAttributeValueTypeBinding)
				{
					...
					{
						gauntletView.BindData(key3, new BindingPath(value6));
					}
				}
			}
		}
