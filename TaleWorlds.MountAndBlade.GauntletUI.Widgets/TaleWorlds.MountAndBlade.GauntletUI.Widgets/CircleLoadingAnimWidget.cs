...
public class CircleLoadingAnimWidget : Widget
{
	...
	private void UpdateMovementValues(float dt)
	{
		if (IsMovementEnabled)
		{
			...
			{
				float num3 = TaleWorlds.Library.MathF.Cos(num2 * ((float)Math.PI / 180f)) * CircleRadius;
				float num4 = TaleWorlds.Library.MathF.Sin(num2 * ((float)Math.PI / 180f)) * CircleRadius;
