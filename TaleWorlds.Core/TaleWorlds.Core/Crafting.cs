...
public class Crafting
{
	...
	private static class CraftedItemGenerationHelper
	{
		private struct CraftingStats
		{
			...
			public static void FillWeapon(ItemObject item, WeaponDescription weaponDescription, WeaponFlags weaponFlags, bool isAlternative, out WeaponComponentData filledWeapon)
			{
				...
				CraftingStats craftingStats = default(CraftingStats);
				craftingStats._craftedData = item.WeaponDesign;
				craftingStats._weaponDescription = weaponDescription;
				CraftingStats craftingStats2 = craftingStats;
				craftingStats2.CalculateStats();
				craftingStats2.SetWeaponData(filledWeapon, isAlternative);
			}
	...
	public void Init()
	{
		...
		{
			if (UsablePiecesList[i] != null)
			{
				array[i] = UsablePiecesList[i].First((WeaponDesignElement p) => !p.CraftingPiece.IsHiddenOnDesigner);
			}
			else
			{
