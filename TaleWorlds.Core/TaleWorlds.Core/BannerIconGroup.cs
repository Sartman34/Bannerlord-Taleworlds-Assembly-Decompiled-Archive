	...
	public void Deserialize(XmlNode xmlNode, MBList<BannerIconGroup> previouslyAddedGroups)
	{
		...
		foreach (XmlNode childNode in xmlNode.ChildNodes)
		{
			if (childNode.Name == "Icon")
			{
				...
				if (!_allIcons.ContainsKey(id2) && !previouslyAddedGroups.Any((BannerIconGroup x) => x.AllIcons.ContainsKey(id2)))
				{
					_allIcons.Add(id2, new BannerIconData(value, textureIndex));
					if (childNode.Attributes["is_reserved"] == null || !Convert.ToBoolean(childNode.Attributes["is_reserved"].Value))
					{
						_availableIcons.Add(id2, new BannerIconData(value, textureIndex));
					}
				}
			}
			else if (childNode.Name == "Background")
			{
				...
				if (!_allBackgrounds.ContainsKey(id) && !previouslyAddedGroups.Any((BannerIconGroup x) => x.AllBackgrounds.ContainsKey(id)))
				{
					_allBackgrounds.Add(id, value2);
				}
			}
		}
	}
