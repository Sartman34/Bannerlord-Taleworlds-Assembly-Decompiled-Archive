...
public class OrderOfBattleVM : ViewModel
{
	...
	private void EnsureAllFormationTypesAreSet(OrderOfBattleFormationItemVM f)
	{
		...
		for (int j = 0; j < _allFormations.Count; j++)
		{
			OrderOfBattleFormationItemVM orderOfBattleFormationItemVM3 = _allFormations[j];
	...
	private List<Tuple<Formation, int, Team.TroopFilter, List<Agent>>> GetMassTransferDataForFormation(OrderOfBattleFormationClassVM formationClass)
	{
		...
		if (num > 0)
		{
			...
			{
				int item = TaleWorlds.Library.MathF.Ceiling((float)allFormationClassesWith[i].Weight / 100f * (float)num);
				list2.Add(item);
			}
			int num2 = 0;
			...
			{
				int num3 = num2 - num;
				list2[(num3 > 0) ? list2.IndexOfMax((int c) => c) : list2.IndexOfMin((int c) => c)] -= Math.Sign(num3);
			}
			for (int j = 0; j < list2.Count; j++)
			{
