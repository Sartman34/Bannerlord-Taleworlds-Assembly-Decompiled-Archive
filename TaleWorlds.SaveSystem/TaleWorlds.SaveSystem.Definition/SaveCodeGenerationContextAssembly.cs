...
internal class SaveCodeGenerationContextAssembly
{
	...
	private void GenerateForClassOrStruct(TypeDefinition typeDefinition)
	{
		...
		for (int i = 0; i < nestedClasses.Length; i++)
		{
			...
			int classGenericInformation = GetClassGenericInformation(text4);
		...
		if (!type.IsGenericTypeDefinition && !type.IsAbstract)
		{
			...
			_managerMethod.AddLine("TaleWorlds.SaveSystem.Definition.CollectObjectsDelegate d" + delegateCount + " = global::" + @namespace + "." + text2 + "." + methodCode.Name + ";");
		...
		foreach (MemberDefinition memberDefinition in typeDefinition.MemberDefinitions)
		{
			...
			else if (!fieldType.IsClass && _definitionContext.GetStructDefinition(fieldType) != null)
			{
				...
				text6 = text6 + "global::" + fullTypeName + ".AutoGeneratedStaticCollectObjects" + usableTypeName + "(this." + name + ", collectedObjects);";
		...
		foreach (MemberDefinition memberDefinition2 in typeDefinition.MemberDefinitions)
		{
			...
			else if (!propertyType.IsClass && _definitionContext.GetStructDefinition(propertyType) != null)
			{
				...
				text8 = text8 + "global::" + fullTypeName2 + ".AutoGeneratedStaticCollectObjects" + usableTypeName2 + "(this." + name2 + ", collectedObjects);";
		...
		foreach (MemberDefinition memberDefinition3 in typeDefinition.MemberDefinitions)
		{
			...
			if (!flag7)
			{
				...
				_managerMethod.AddLine("{");
				_managerMethod.AddLine("var memberDefinition = typeDefinition" + delegateCount + "." + text10 + "(new global::TaleWorlds.SaveSystem.Definition.MemberTypeId(" + memberDefinition3.Id.TypeLevel + "," + memberDefinition3.Id.LocalSaveId + "));");
				string text11 = "global::" + @namespace + "." + text2 + "." + methodCode3.Name;
				_managerMethod.AddLine("memberDefinition.InitializeForAutoGeneration(" + text11 + ");");
				_managerMethod.AddLine("}");
				_managerMethod.AddLine("");
			}
		}
