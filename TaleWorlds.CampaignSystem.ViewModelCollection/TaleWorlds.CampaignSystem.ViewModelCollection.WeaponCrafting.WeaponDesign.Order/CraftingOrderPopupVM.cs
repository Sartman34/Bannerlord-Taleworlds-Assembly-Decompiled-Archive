...
public class CraftingOrderPopupVM : ViewModel
{
	...
	public void RefreshOrders()
	{
		...
		if (craftingOrderSlots != null)
		{
			...
			{
				List<CraftingStatData> orderStatDatas = _getOrderStatDatas(list[i]).ToList();
				CampaignUIHelper.IssueQuestFlags questFlagsForOrder = GetQuestFlagsForOrder(list[i]);
				CraftingOrders.Add(new CraftingOrderItemVM(list[i], SelectOrder, _getCurrentCraftingHero, orderStatDatas, questFlagsForOrder));
				issueQuestFlags |= questFlagsForOrder;
			}
			QuestType = (int)issueQuestFlags;
			for (int j = 0; j < list2.Count; j++)
			{
				List<CraftingStatData> orderStatDatas2 = _getOrderStatDatas(list2[j]).ToList();
				CraftingOrders.Add(new CraftingOrderItemVM(list2[j], SelectOrder, _getCurrentCraftingHero, orderStatDatas2));
			}
