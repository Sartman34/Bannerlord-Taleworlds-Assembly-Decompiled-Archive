...
public class WeaponDesignVM : ViewModel
{
	...
	public void ExecuteUndo()
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			{
				CraftingPieceVM piece2 = _pieceListsDictionary[pieceTypes].Pieces.First((CraftingPieceVM piece) => piece.CraftingPiece.CraftingPiece == _crafting.SelectedPieces[i].CraftingPiece);
				OnSetItemPiece(piece2);
			}
		}
		RefreshItem();
		_updatePiece = true;
	...
	public void SelectWeapon(ItemObject itemObject)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			if (_crafting.CurrentCraftingTemplate.IsPieceTypeUsable(pieceTypes))
			{
				CraftingPieceVM piece2 = _pieceListsDictionary[pieceTypes].Pieces.First((CraftingPieceVM piece) => piece.CraftingPiece.CraftingPiece == _crafting.CurrentWeaponDesign.UsedPieces[i].CraftingPiece);
				OnSetItemPiece(piece2, _crafting.CurrentWeaponDesign.UsedPieces[i].ScalePercentage);
			}
		}
		RefreshItem();
		AddHistoryKey();
