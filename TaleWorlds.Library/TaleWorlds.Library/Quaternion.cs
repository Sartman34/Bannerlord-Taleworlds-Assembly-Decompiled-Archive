...
public struct Quaternion
{
	...
	public Quaternion TransformToParent(Quaternion q)
	{
		...
		result.X = Y * q.Z - Z * q.Y + W * q.X + X * q.W;
		result.Y = Z * q.X - X * q.Z + W * q.Y + Y * q.W;
		result.Z = X * q.Y - Y * q.X + W * q.Z + Z * q.W;
	...
	{
		Quaternion result = default(Quaternion);
		result.X = Z * q.Y - Y * q.Z + W * q.X - X * q.W;
		result.Y = X * q.Z - Z * q.X + W * q.Y - Y * q.W;
	...
	public static Vec3 EulerAngleFromQuaternion(Quaternion quat)
	{
		...
		result.z = MathF.Atan2(2f * (x * y + z * w), num2 - num3 - num4 + num);
		result.x = MathF.Atan2(2f * (y * z + x * w), 0f - num2 - num3 + num4 + num);
		result.y = MathF.Asin(-2f * (x * z - y * w));
		return result;
	}
