...
public static class TauntUsageManager
{
	...
	private static List<TauntUsageSet> _tauntUsageSets;

	private static Dictionary<string, int> _tauntUsageSetIndexMap;
	...
	{
		foreach (XmlNode item in LoadXmlFile(ModuleHelper.GetModuleFullPath("Native") + "ModuleData/Multiplayer/taunt_usage_sets.xml").DocumentElement.SelectNodes("taunt_usage_set"))
		{
	...
	public static TauntUsageSet GetUsageSet(string id)
	{
	...
	public static TauntUsage.TauntUsageFlag GetIsActionNotSuitableReason(int index, bool isLeftStance, bool onFoot, WeaponComponentData mainHandWeapon, WeaponComponentData offhandWeapon)
	{
