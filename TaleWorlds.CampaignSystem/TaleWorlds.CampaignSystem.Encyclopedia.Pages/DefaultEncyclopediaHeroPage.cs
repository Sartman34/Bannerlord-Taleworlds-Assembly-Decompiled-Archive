	...
	protected override IEnumerable<EncyclopediaListItem> InitializeListItems()
	{
		...
		foreach (Hero hero2 in Hero.AllAliveHeroes)
		{
			if (IsValidEncyclopediaItem(hero2) && !hero2.IsNotable && hero2.Age >= (float)comingOfAge)
			{
				...
				{
					name = hero2.Name.ToString();
				}
				yield return new EncyclopediaListItem(hero2, name, "", hero2.StringId, GetIdentifier(typeof(Hero)), CanPlayerSeeValuesOf(hero2), delegate
				{
		...
		foreach (Hero hero in Hero.DeadOrDisabledHeroes)
		{
			...
			{
				yield return new EncyclopediaListItem(hero, hero.Name.ToString(), "", hero.StringId, GetIdentifier(typeof(Hero)), CanPlayerSeeValuesOf(hero), delegate
				{
					InformationManager.ShowTooltip(typeof(Hero), hero, false);
				});
			}
