	...
	public class ArtisanOverpricedGoodsIssueQuest : QuestBase
	{
		...
		protected override void SetDialogs()
		{
			...
			DiscussDialogFlow = DialogFlow.CreateDialogFlow("quest_discuss").NpcLine(textObject).Condition(() => Hero.OneToOneConversationHero == base.QuestGiver)
				...
				.Condition(delegate
				{
					int availableRequestedItemCountOnPlayer2 = GetAvailableRequestedItemCountOnPlayer();
					Campaign.Current.ConversationManager.GetCurrentDialogLine().SetTextVariable("REQUESTED_GOOD", _requestedTradeGood.Name);
					Campaign.Current.ConversationManager.GetCurrentDialogLine().SetTextVariable("AMOUNT_TO_DELIVER", availableRequestedItemCountOnPlayer2);
					return availableRequestedItemCountOnPlayer2 > 0 && availableRequestedItemCountOnPlayer2 < _requestedTradeGoodAmount - _givenTradeGoods;
				})
