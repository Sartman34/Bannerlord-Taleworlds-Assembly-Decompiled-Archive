...
public class MerchantArmyOfPoachersIssueBehavior : CampaignBehaviorBase
{
	...
	public class MerchantArmyOfPoachersIssueQuest : QuestBase
	{
		...
		private TextObject _questSuccessPlayerComesToAnAgreementWithPoachers => new TextObject("{=qPfJpwGa}You have persuaded the poachers to leave the district.");
		...
		private TextObject _questFailedWithTimeOutLogText => new TextObject("{=HX7E09XJ}You failed to complete the quest in time.");
		...
		private bool poachers_persuasion_clickable_option_5_on_condition(out TextObject hintText)
		{
			...
			if (_task.Options.Count > 4)
			{
				hintText = (_task.Options.ElementAt(4).IsBlocked ? hintText : TextObject.Empty);
		...
		{
			_poachersParty = MobileParty.CreateParty("poachers_party", null);
			TextObject customName = new TextObject("{=WQa1R55u}Poachers Party");
			_poachersParty.InitializeMobilePartyAtPosition(new TroopRoster(_poachersParty.Party), new TroopRoster(_poachersParty.Party), _questVillage.Settlement.GetPosition2D);
			...
			_poachersParty.ItemRoster.AddToCounts(@object, num * 2);
			...
			_poachersParty.ActualClan = actualClan;
		...
		{
			AddLog(_questFailedAfterTalkingWithProachers);
		...
		{
			AddLog(_questSuccessPlayerComesToAnAgreementWithPoachers);
		...
		{
			AddLog(_questFailWithPlayerDefeatedAgainstPoachers);
		...
		{
			AddLog(_questSuccessWithPlayerDefeatedPoachers);
			TraitLevelingHelper.OnIssueSolvedThroughQuest(base.QuestGiver, new Tuple<TraitObject, int>[1]
			{
				new Tuple<TraitObject, int>(DefaultTraits.Honor, 50)
			});
		...
		{
			AddLog(_questFailedWithTimeOutLogText);
			TraitLevelingHelper.OnIssueSolvedThroughQuest(base.QuestGiver, new Tuple<TraitObject, int>[1]
			{
				new Tuple<TraitObject, int>(DefaultTraits.Honor, -30)
			});
		...
		{
			AddLog(_questCanceledTargetVillageRaided);
			CompleteQuestWithFail();
		}
