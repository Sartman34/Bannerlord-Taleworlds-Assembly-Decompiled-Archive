	...
	public class ArtisanCantSellProductsAtAFairPriceIssueQuest : QuestBase
	{
		...
		private DialogFlow GetDeliveryDialogFlow()
		{
			...
			return DialogFlow.CreateDialogFlow("hero_main_options", 125).PlayerLine(textObject).Condition(delegate
			{
				...
				.Condition(delegate
				{
					int availableRequestedItemCountOnPlayer2 = GetAvailableRequestedItemCountOnPlayer(_rawMaterialsToBeDelivered);
					int num2 = _amountOfRawGoodsToBeDelivered - _deliveredRawGoods;
					Campaign.Current.ConversationManager.GetCurrentDialogLine().SetTextVariable("AVAILABLE_AMOUNT", availableRequestedItemCountOnPlayer2);
