...
public class DefaultSmithingModel : SmithingModel
{
	...
	private List<(ItemQuality quality, float probability)> GetModifierQualityProbabilities(WeaponDesign weaponDesign, Hero hero)
	{
		...
		{
			(ItemQuality, float) tuple2 = list[i];
			list[i] = (tuple2.Item1, tuple2.Item2 / num2);
		}
	...
	private static void AdjustModifierProbabilities(List<(ItemQuality quality, float probability)> modifierProbabilities, ItemQuality qualityToAdjust, float amount, List<ItemQuality> qualitiesToIgnore)
	{
		...
		{
			(ItemQuality, float) tuple2 = modifierProbabilities[i];
			if (tuple2.Item1 == qualityToAdjust)
			{
				modifierProbabilities[i] = (tuple2.Item1, tuple2.Item2 + amount);
			}
			else if (!qualitiesToIgnore.Contains(tuple2.Item1))
			{
				float num4 = tuple2.Item2 - (num2 + num3);
				if (num4 < 0f)
				{
					num3 = 0f - num4;
					num4 = 0f;
				...
				modifierProbabilities[i] = (tuple2.Item1, num4);
			}
		}
		float num5 = modifierProbabilities.Sum(((ItemQuality quality, float probability) tuple) => tuple.probability);
		for (int j = 0; j < modifierProbabilities.Count; j++)
		{
			(ItemQuality, float) tuple3 = modifierProbabilities[j];
			modifierProbabilities[j] = (tuple3.Item1, tuple3.Item2 / num5);
		}
	}
