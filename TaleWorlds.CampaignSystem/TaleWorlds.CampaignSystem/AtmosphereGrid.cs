...
public class AtmosphereGrid
{
	...
	public AtmosphereState GetInterpolatedStateInfo(Vec3 pos)
	{
		...
		{
			AtmosphereStateSortData item = default(AtmosphereStateSortData);
			item.Position = state.Position;
			item.InitialIndex = num++;
			list.Add(item);
		}
		pos.z *= 0.3f;
		...
		foreach (AtmosphereStateSortData item2 in list)
		{
			AtmosphereState atmosphereState2 = states[item2.InitialIndex];
