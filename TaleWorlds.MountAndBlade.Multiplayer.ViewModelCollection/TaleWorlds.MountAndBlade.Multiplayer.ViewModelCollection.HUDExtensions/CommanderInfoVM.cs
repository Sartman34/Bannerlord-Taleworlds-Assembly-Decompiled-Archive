...
public class CommanderInfoVM : ViewModel
{
	...
	{
		foreach (CapturePointVM allyControlPoint in AllyControlPoints)
		{
			allyControlPoint.Refresh(0f, 0f, 0f);
		}
		foreach (CapturePointVM enemyControlPoint in EnemyControlPoints)
		{
			enemyControlPoint.Refresh(0f, 0f, 0f);
		}
		foreach (CapturePointVM neutralControlPoint in NeutralControlPoints)
		{
	...
	private void InitCapturePoints()
	{
		if (_commanderInfo != null && GameNetwork.MyPeer?.GetComponent<MissionPeer>()?.Team != null)
		{
			...
			{
				CapturePointVM capturePointVM = new CapturePointVM(flagCapturePoint, (TargetIconType)(17 + flagCapturePoint.FlagIndex));
				HandleAddNewCapturePoint(capturePointVM);
			}
	...
	public void OnTeamChanged()
	{
		...
		if (_allyTeam != null)
		{
			IEnumerable<Team> source = Mission.Current.Teams.Where((Team t) => t.IsEnemyOf(_allyTeam));
	...
	{
		CapturePointVM capturePointVM = AllyControlPoints.SingleOrDefault((CapturePointVM c) => c.Target == target);
		if (capturePointVM != null)
		{
			return capturePointVM;
		}
		CapturePointVM capturePointVM2 = EnemyControlPoints.SingleOrDefault((CapturePointVM c) => c.Target == target);
		if (capturePointVM2 != null)
		{
			return capturePointVM2;
		}
		CapturePointVM capturePointVM3 = NeutralControlPoints.SingleOrDefault((CapturePointVM c) => c.Target == target);
