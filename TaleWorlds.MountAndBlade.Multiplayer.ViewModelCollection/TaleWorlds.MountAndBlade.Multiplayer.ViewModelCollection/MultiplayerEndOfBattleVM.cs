...
public class MultiplayerEndOfBattleVM : ViewModel
{
	...
	private void OnEnabled()
	{
		...
		foreach (MissionScoreboardComponent.MissionScoreboardSide item in missionBehavior.Sides.Where((MissionScoreboardComponent.MissionScoreboardSide s) => s != null && s.Side != BattleSideEnum.None))
		{
	...
	private int GetPeerScore(MissionPeer peer)
	{
		...
		{
			return peer.GetComponent<DuelMissionRepresentative>()?.Score ?? 0;
		}
		return peer.Score;
	}
}
